...
int OptimizationProcessor::run()
{
    ...
    try {
        ...
        if(!featureDetector.compare("sift")) {
            ...
            if(!mArguments["fmask"].empty()) {
                ...
                if(!maskType.compare("vessel")) {
                    ...
                    optPipe.addFeatureDetectionStep(new pl::SiftDetector(fdCfg), vesselMask);
                }
            } else {
                // Without mask
                optPipe.addFeatureDetectionStep(new pl::SiftDetector(fdCfg), cv::Ptr<pl::MaskGenerator>());
            }
        }
        ...
        if(!featureExtractor.compare("sift")) {
            ...
            feCfg->fromJSON(file);
            // Without mask
            optPipe.addFeatureExtractionStep(new pl::SiftExtractor(feCfg), cv::Ptr<pl::MaskGenerator>());
        }
    ...
    //    Dimensionality reduction
        ...
        if(!reductionMethod.compare("pca")) {
            ...
            pca->fromJSON(file);
            optPipe.addDimensionalityReductionStep(new pl::PCAStep(pca));
        }
    ...
    //    Encoding
        ...
        if(!encodingMethod.compare("vlad")) {
            ...
            optPipe.addEncodingStep(new pl::VladEncodingStep(encoding));
        }
    ...
    //    Learning algorithm
        ...
        if(!learningAlgo.compare("sgd")) {
            ...
            sgd->fromJSON(file);
            optPipe.addClassificationStep(new pl::SGDStep(sgd));
        }
